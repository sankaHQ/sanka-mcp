...
use the existing authenticated SDK `get` transport, so no generated SDK resource,
API implementation or API contract change is needed for this phase.

This group adds only GET operations. Creating migrations, starting scans/plans,
applying a reviewed plan, job control, starting verification, ingestion mutations
and code migration operations remain separate follow-up phases. Matching plugin
skills should be packaged when these tools are released.
