// Search engine behind the search_docs tool. Method entries come from generated/sdk-method-docs.ts,
// which scripts/generate-sdk-method-docs.ts builds from the TypeScript client in src/resources.

import MiniSearch from 'minisearch';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import { sdkMethodDocs, type SdkMethodDoc } from './generated/sdk-method-docs';
...
const EMBEDDED_METHODS: MethodEntry[] = sdkMethodDocs.map(methodEntryFromDoc);

function methodEntryFromDoc(doc: SdkMethodDoc): MethodEntry {
  const resource = doc.qualified.split('.').slice(1, -1).join('.');
  const method = doc.qualified.slice(doc.qualified.lastIndexOf('.') + 1);
  return {
    ...doc,
    resourcePath: `(resource) ${resource} > (method) ${method}`,
    markdown: methodMarkdown(doc),
  };
}

function methodMarkdown(doc: SdkMethodDoc): string {
  const sections = [
    `## ${doc.name}`,
    `\`${doc.signature}\``,
    `**${doc.httpMethod}** \`${doc.endpoint}\``,
    doc.description,
  ];
  if (doc.params.length > 0) {
    sections.push(
      `### Parameters\n\n${doc.params.map((param) => `- \`${param.replace(/;$/, '')}\``).join('\n')}`,
    );
  }
 ...
 * Method data is generated from the TypeScript client; prose documents
